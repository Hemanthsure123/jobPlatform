...
class Jobs extends Component {
  ...
  getJobs = async () => {
    ...
    const {employeeTypeList, minimumSalary, searchInput, locationBasedList} =
      this.state
    ...
    const apiUrl = `https://apis.ccbp.in/jobs?employment_type=${employeeTypeList.join()}&location=${locationBasedList.join()}&minimum_package=${minimumSalary}&search=${searchInput}`
  ...
  changeLocationList = type => {
    const {locationBasedList} = this.state

    const inputNotInList = locationBasedList.filter(
      eachItem => eachItem === type,
    )
    // console.log(inputNotInList)
    if (inputNotInList.length === 0) {
      this.setState(
        prevState => ({
          locationBasedList: [...prevState.locationBasedList, type],
        }),
        this.getJobs,
      )
    } else {
      const filteredData = locationBasedList.filter(
        eachItem => eachItem !== type,
      )
      // console.log(filteredData)

      this.setState({locationBasedList: filteredData}, this.getJobs)
    }
  }
